...
# Tutorial moved to commonsense.tutorial — see tutorial/legacy.py for the
# Phase 1 fallback (this same modal). Phase 2+ adds spotlight + mascot.
...
class EditorApp:
    ...
    def _on_key(self, event: tk.Event) -> None:  # type: ignore[type-arg]
        ...
        if event.char.isdigit():
            ...
            if 0 <= i < len(self.model.palette):
                ...
                self._update_status()
                self.events.emit("color_picked", index=i, source="hotkey")
            return
        ch = event.char.lower()
        actions = {
            "c": self._clear_frame,
    ...
    def _canvas_press(self, event: tk.Event) -> None:  # type: ignore[type-arg]
        ...
        if self.tool == TOOL_PICKER:
            ...
            self.events.emit("color_picked", index=self.selected_color, source="picker")
            return
    ...
    def _paint_at(self, x: int, y: int) -> None:
        ...
        self._push_hat()
        # Tutorial signal — drag-painting the same cell is filtered above so
        # this fires once per distinct (x, y) per drag.
        self.events.emit("paint", x=x, y=y, idx=idx)
    ...
    def _select_color(self, i: int) -> None:
        ...
        self._render_palette()
        self._update_status()
        self.events.emit("color_picked", index=i)
    ...
    def _add_frame(self) -> None:
        ...
        self.events.emit("frame_added", count=len(self.model.frames))
    ...
    def _toggle_play(self) -> None:
        ...
        if self.playing:
            ...
            # Only emit on the leading edge — pause→play, not play→pause —
            # so the gate fires when the kid actually starts playback.
            self.events.emit("play_started")
        else:
    ...
    def _do_save(self, path: str) -> None:
        ...
        try:
            ...
            # Only emits on success — failed saves don't satisfy the gate.
            self.events.emit("save_done", path=path)
